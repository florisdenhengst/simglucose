...
Action = namedtuple("patient_action", ["CHO", "insulin", "exercise_intensity"])
...
class T1DExercisePatient(Patient):
    ...
    def reset(self):
        ...
        self._last_action = Action(CHO=0, insulin=0, exercise_intensity=0)
        self.is_eating = False
        self.planned_meal = 0
...
if __name__ == "__main__":
    ...
    while p.t < 1000:
        ...
        # if p.t == 150:
        #     ins = 80.0 / 12.0 + basal
        act = Action(insulin=ins, CHO=carb, exercise_intensity=0)
        t.append(p.t)
        CHO.append(act.CHO)
...
# Updated Action to include exercise intensity (0.0 to 1.0)
Action = namedtuple("patient_action", ["CHO", "insulin", "exercise_intensity"])
Observation = namedtuple("observation", ["Gsub"])
...
class T1DPatient(Patient):
    ...
    def __init__(self, params, init_state=None, random_init_bg=False, seed=None, t0=0):
        ...
        self._seed = seed
        self.t0 = t0
        self.current_exercise_intensity = 0.  # Internal state for exercise, interpreted as %VO2max (0.0 to 1.0)
        self.reset()
    ...
    def activity_map(self, activity_type):
        """
        Maps human-readable activity to intensity levels.
        Returns intensity scale (0.0 to 1.0)
        """
        mapping = {
            "rest": 0.0,
            "walking": 0.2,
        ...
        return mapping.get(activity_type.lower(), 0.0)

    def step(self, action):
        # Handle Exercise Announcement/Suggestion
        # If exercise is a string (e.g., 'jogging'), map it; if numeric, use directly
        if "exercise_intensity" in action._fields:
            self.current_exercise_intensity = action.exercise_intensity
        elif "exercise" in action._fields:
            self.current_exercise_intensity = self.activity_map(action.exercise)
        else:
            self.current_exercise_intensity = 0.0

        # Convert announcing meal
        to_eat = self._announce_meal(action.CHO)
        action = action._replace(CHO=to_eat)

        if action.CHO > 0 and self._last_action.CHO <= 0:
            logger.info(f"t = {self.t}, patient starts eating ...")
        ...
        if action.CHO <= 0 and self._last_action.CHO > 0:
            logger.info(f"t = {self.t}, Patient finishes eating!")
            self.is_eating = False

        self._last_action = action

        # Pass intensity to the ODE solver
        self._odesolver.set_f_params(
            action, self._params, self._last_Qsto, self._last_foodtaken, self.current_exercise_intensity
        )
    ...
    @staticmethod
    def model(t, x, action, params, last_Qsto, last_foodtaken, intensity):
        """
        Refined Exercise Model for UVA/Padova
        intensity: 0.0 to 1.0 (representing 0% to 100% VO2max)
        """
        dxdt = np.zeros(13)
        d = action.CHO * 1000  # g -> mg
        # Insulin conversion
        ...
        # --- 1. DYNAMIC EXERCISE KINETICS ---
        # In a production environment, you should add a 14th state for 'exercise_eff'
        # to handle the slow rise/fall. Here, we calculate a quasi-steady state 
        # based on intensity with a safety buffer.
        
        # Scale Vmt based on %VO2max. 
        # Realistic delta Vmt is ~0.25 at 60% intensity (0.6)
        delta_Vmt = 0.4 * intensity 
        
        # Increase insulin sensitivity (recruitment of GLUT4)
        # Exercise can increase insulin-dependent utilization by 2x-4x
        sens_multiplier = 1.0 + (2.0 * intensity)
        
        # --- 2. STOMACH & INTESTINE KINETICS ---
        qsto = x[0] + x[1]
        ...
        # Exercise naturally increases EGP to compensate for muscle uptake.
        # Without this, the model will drop too fast at high intensities.
        exercise_EGP_boost = 1.0 + (0.5 * intensity) 
        EGPt = (params.kp1 * exercise_EGP_boost) - params.kp2 * x[3] - params.kp3 * x[8]
        
        Uiit = params.Fsnc

        # Renal excretion
        Et = max(params.ke1 * (x[3] - params.ke2), 0) if x[3] > params.ke2 else 0

        # --- 4. GLUCOSE KINETICS (Plasma & Tissue) ---
        # Plasma Glucose
        dxdt[3] = max(EGPt, 0) + Rat - Uiit - Et - params.k1 * x[3] + params.k2 * x[4]
        dxdt[3] = (x[3] >= 0) * dxdt[3]

        # UTILIZATION (The Core Change)
        # Vmt = Resting + Delta (based on intensity)
        # We apply the sensitivity multiplier to the insulin-dependent part (x[6])
        Vmt = params.Vm0 + (params.Vmx * sens_multiplier * x[6]) + delta_Vmt
        ...
        # Insulin Action (x[6] is the remote insulin compartment)
        dxdt[6] = -params.p2u * x[6] + params.p2u * (It - params.Ib)
        dxdt[7] = -params.ki * (x[7] - It)
        dxdt[8] = -params.ki * (x[8] - x[7])
    ...
    def reset(self):
        ...
        self._last_foodtaken = 0
        self.name = self._params.Name
        self.current_exercise_intensity = 0
